...
|---|---|
| Access / portability | `GET /api/account/export` — one JSON document with account, profile, resumes, jobs, applications, emails, vault (decrypted for the owner), settings, logs and audit trail. Also downloadable from the UI (**Account → Download JSON export**). |
| Erasure | `DELETE /api/account?confirm=<email>` — hard-deletes every owned row **and** the resume files on disk; an audit row recording the deletion is retained with the user id detached (legitimate-interest record of the erasure itself). |
