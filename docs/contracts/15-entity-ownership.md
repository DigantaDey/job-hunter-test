...
| **Application tracking** ✓ shipped | `application_tracking` | `app/services/application_tracking.py` | `/tracking` board and agenda, the job drawer panel, `GET /api/analytics/performance`, reports | `open_tracking` on the first lifecycle write for a `(user, job)`, or `observe_submission` from the apply flow | state transitions only ([07 §11](07-application-state-machine.md)); snapshots are frozen at open and refreshed only by an explicit event | no terminal row — the record lives as long as the job it describes | deleted |
| **Application tracking event** ✓ shipped | `application_tracking_events` | same | the timeline, `by_origin` in reports, support | every fact: transition, note, follow-up, correction, snapshot refresh, provisional suggestion | never | append-only; `sequence` gapless per record | deleted |
