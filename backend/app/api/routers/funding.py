...
async def refresh(user: CurrentUser, db: DbSession, payload: dict = Body(default={})):
    ...
    window = int(payload.get("window_days") or get_setting(db, user.id, "funding", "freshness_days", settings.funding_freshness_days))
    provider = payload.get("provider") or get_setting(db, user.id, "funding", "provider", settings.funding_provider)
    item = enqueue(
        db,
        user_id=user.id,
        pipeline="funding",
        payload={"window_days": window, "provider": provider, "stages": payload.get("stages"), "user_context": _user_funding_context(db, user.id)},
        priority=5,
        dedupe_key=f"funding:{provider}:{window}",
    )
    return {"queued": item is not None, "pipeline_job_id": item.id if item else None, "duplicate": item is None, "provider": provider, "window_days": window, "context_source": context.get("source")}
...
async def process_company(company_name: str, request: Request, user: CurrentUser, db: DbSession):
    ...
    if row.has_open_positions:
        ...
        score, reason = await ai_score(profile.data or {}, f"Engineering role at {row.name} ({row.stage}, {row.industry}). {row.summary}")
        job = Job(
            user_id=user.id,
