"""Credential vault endpoints (list / export / reveal / delete forever)."""
...
def create_entry(request: Request, user: CurrentUser, db: DbSession, payload: dict):
    ...
    audit.audit(db, "vault.credential_created", user=user, target=domain,
                detail={"entry_id": entry.id, "origin": "manual"}, request=request)
...
def reveal(entry_id: int, request: Request, user: CurrentUser, db: DbSession):
    ...
    audit.audit(db, "vault.viewed", user=user, target=entry.domain,
                detail={"entry_id": entry.id, "revealed": True}, request=request)
    return {"id": entry.id, "domain": entry.domain, "username": entry.username,
            "password": reveal_password(entry)}
...
def export_chrome(request: Request, user: CurrentUser, db: DbSession):
    ...
    audit.audit(db, "vault.exported", user=user, target="chrome",
                detail={"entries": len(entries)}, request=request)
    return PlainTextResponse(
        export_chrome_csv(entries), media_type="text/csv",
...
def export_apple(request: Request, user: CurrentUser, db: DbSession):
    entries = list_vault_entries(db, user.id)
    audit.audit(db, "vault.exported", user=user, target="apple",
                detail={"entries": len(entries)}, request=request)
    return PlainTextResponse(
        export_apple_csv(entries), media_type="text/csv",
