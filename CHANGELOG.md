...
**The call log (`app/services/ai_log.py`, model `AICallRecord`).** Every provider call now stores what went on the wire: the outbound request **body per attempt** (so an escalation is visible as an escalation: attempt 1 at 1200 tokens, attempt 2 at 3400), the endpoint and routing parameters, the outcome, tokens/cost, latency and a bounded excerpt of the answer. The bodies are captured at send time (`ai_client._capture_request`) and are *the same object* the provider received — the tests drive the real gateway against the scripted provider and diff the stored body against the wire. Failures are first-class: `reason` and `http_status` are recorded, so "the guardrail rejected it" and "the provider 503'd" are distinguishable after the fact instead of both reading `error`. Deliberately bounded and private: only bodies are stored (headers, and therefore the API key, are never captured; the bodies pass through the gateway's own secret scrubber, now applied to every string in the structure, and parameters are scrubbed too), prompt text is clipped to `AI_LOG_PROMPT_CHARS` (20000) and answers to `AI_LOG_RESPONSE_CHARS` (2000) with the clip **marked in the payload** (`_truncated` / `_original_chars`) so a clipped prompt is never mistaken for the whole prompt, and only the last three attempt bodies are kept (`_MAX_LOGGED_ATTEMPTS`).

**The decision log (model `LayaDecision`).** One row per local-engine forward pass: task (`ranking` / `classification` / `field_mapping`), status (`ok` / `low_confidence` / `timeout` / `error` / `parked`), which checkpoint answered, question count, the calibrated confidence **against the active floor** (so a below-floor `auto` pass is recorded as the escalation it caused), whether `laya_only` was in force, latency, and the compact per-question verdict (choice / level / confidence — never the state that was scored, which is the call log's job). The three call sites now pass their task through (`scoring.laya_score_detailed`, `classifier.laya_company_size`, `assisted_fill._laya_map_fields`), and `laya.predict` records whatever happened, including the failures that never reach a caller: `engine_parked` and `predict_failed` are now visible as themselves rather than as "the AI gateway answered".

**Owner-only, and only owner-only.** A new `GET /api/admin/ai/calls`, `/ai/calls/{id}` (the bodies), `/ai/stats`, `/api/admin/laya/decisions`, `/laya/stats` live behind the existing router-level `require_owner` — a member token gets 403 `owner_required` before any handler runs, the routes are in `tests/test_role_separation.py`'s pinned owner-only list, and there is no user-facing variant. `GET /api/admin/overview` carries both 24 h rollups (`ai_log`, `laya`) so the console's single pane answers "is anything failing right now?". The new **Admin → AI log** page (`/admin/ai`, in `ADMIN_ROUTES` behind `<RequireOwner>`) renders the Laya status mix per task with latency/confidence, the call list with filters (status/workflow/window), and — on *Exact request* — every attempt body plus the answer excerpt, pretty-printed.

**Bounded twice, and never in the way.** `AI_LOG_RETENTION_DAYS` (7) deletes old rows, one sweep per `_PRUNE_INTERVAL_SECONDS` (300 s) triggered on write and batched (`AI_LOG_PRUNE_BATCH`) so retention never holds a transaction open (the sweep's "never run" sentinel is `None`, not `0.0` — `time.monotonic()` counts from boot, so a numeric zero means "300 s ago" only on a host that has been up longer than the interval, and a freshly booted CI runner or pod would have skipped its first sweep); `AI_LOG_ENABLED=false` restores the previous behaviour exactly (no row, no query, no read). Every write is wrapped: an observability table that can fail an AI call is worse than no observability. Both tables carry a real `user_id` FK, so account erasure reaches them through the existing schema-derived plan (`tests/test_deletion_integrity.py` seeds them).

Covered by `backend/tests/test_ai_log.py` (**18 tests** — the body stored is the body sent, escalation visible as attempts, failures with reason/status, member 403 + anonymous 401 on every route, the Laya status vocabulary including timeout/parked/low-confidence, retention and pruning, clipping and secret masking, and "a broken sink does not fail the call") and `frontend/src/pages/__tests__/AdminAILogs.test.tsx` (**5 tests** — the Laya panel, the honest disabled state, the exact-request drill-down, filters).

### Added — the shared job pool: every run's findings become everyone's coverage, for 7 days

The complaint behind this: *"job discovery is still very slow, and I doubt our system covers almost all the jobs available in the market."* Both halves had the same root cause — every account's discovery run was a **cold start** that re-fetched the same public postings, so a brand-new user waited minutes for a board while the workspace had already seen thousands of the same jobs.

**The pool (`app/services/job_pool.py`, models `JobPoolEntry` / `JobPoolSeen` / `JobPoolMetric`, migration `m4n5o6p7q8r9`).** Every discovery run now folds the postings it scanned into one shared, deduplicated corpus keyed by the *same* canonical identity the board uses (`source:external_id`, else `company|title`), so a pool-sourced row merges with a live-sourced row instead of duplicating it. The pool has three deliberate boundaries: **retention is 7 days** (`JOB_POOL_RETENTION_DAYS`, `expires_at = last_seen_at + retention`) and pruning *deletes the content* — no title, no description, no URL, no payload; a pruned **or withdrawn** posting leaves only an aggregate row (`job_pool_metrics`: counters, a SHA-256 identity, the public company name as the churn dimension) behind. **No tenant column on the entry** — a posting is public market data; the person-level link is `job_pool_seen` (a real `user_id` FK), which is why account erasure reaches the pool with no second hand-written table list (the schema-derived plan already covers it, and `tests/test_deletion_integrity.py` pins the seed). **Off means off**: `JOB_POOL_ENABLED=false` reads nothing, writes nothing and matches nothing — the pre-pool report, byte for byte. Runs report what they did (`report["pool"]`: `offered` / `ingested` / `refreshed` / `pruned` / `live_entries` / `contributors`), so the coverage claim is visible in the run log rather than inferred.

**Instant match on onboarding completion.** `job_pool.maybe_instant_match` is called at the moment onboarding reaches its terminal state (`apply_extraction_success`) and again after a profile-review decision activates (or corrects) the profile — both post-commit and failure-isolated, so a pool problem can never fail onboarding. It writes the pool's best current matches to the board **immediately** with deterministic `preliminary` scores (zero network, zero provider calls, honestly labelled), queues the user's own live discovery run behind them (`priority=3`, dedupe `discovery:instant:{persona}`, `trigger=instant_match`) and is idempotent per *(profile, document)* — the marker is a document fingerprint, not just a profile id, so re-resolving a field looks again while an unchanged one is a no-op that cannot re-insert a job. The board's `jobs_max` cap and the discovery quota are respected and reported as `skipped` (`jobs_cap_reached` / `quota` / `pool_empty` / `nothing_new`), never raised.

**Owner-only aggregates.** `GET /api/admin/overview` gains a `job_pool` block (`metrics_snapshot`): live entries, distinct companies, contributors, entries by source, and the `gone` rollup — pruned/deleted counts by reason, `times_seen`, `max_distinct_users`, churn by company. **Admin → Shared job pool** renders it (live corpus, retention window, per-source split, the *Gone — aggregated only* list and the churn line), and the same panel says *Pool disabled* when `JOB_POOL_ENABLED=false` rather than implying a corpus the deployment does not keep. The route-level `require_owner` is the access rule (a member gets 403 before the handler runs), the payload is counters only, and a pin test fails if any posting content column is ever added to the metric table. A source that reports a posting **closed/withdrawn** takes the same route immediately — content out, `job_pool_metrics(reason="deleted")` in — and the run only contributes postings it actually considers live, so a dead listing is neither advertised nor re-ingested by the run that just dropped it.

Covered by `backend/tests/test_job_pool.py` (**19 tests** — contribution and cross-user coverage, "pool-supplied candidates are not re-counted", retention/deletion aggregates and the content-free schema pin, the owner-only snapshot, the instant-match gates and idempotency, the onboarding end-to-end path), `tests/test_deletion_integrity.py` (the pool's `user_id` link joins the schema-derived erasure seed), `tests/test_laya_engine.py` (**17 tests**, including parking) and `frontend/src/pages/__tests__/AdminJobPool.test.tsx` (**2 tests**).

### Fixed — the token-limit policy was smaller than the answer it asked for

The logs showed the same verdict burning three round trips: `AI scoring: the JSON was cut off at 1200 tokens — retrying with 2400` → again at 2400 → 4800. The ceiling was the bug: `SCORING_OUTPUT_TOKENS` was a hand-picked **1200**, while the prompt demands `SCORE_SCHEMA`'s own maxima (reason ≤600 chars, recommendation reason ≤400, up to 15 strengths / 12 missing skills / 8 evidence quotes at ≤300 chars) — roughly 1.6k tokens of JSON before a reasoning model thinks at all. It is now **derived from the schema** (`ai_guardrails.schema_output_budget`, `SCORE_SCHEMA` + its list caps → 3400), so the budget can never silently drift below the contract again; the ceiling is not a spend (a model that answers in 600 tokens still bills 600), the user's output ceiling and the escalation hard stop still clamp it, and `tests/test_ai_token_budgets.py` now pins the properties (first attempt = the workflow budget, strictly increasing retries, past the old 16000 clamp) instead of a frozen chain.

### Fixed — discovery's AI slices were serial, and a down local engine cost one timeout per candidate

Scoring and company-size classification ran one candidate at a time: twelve verdicts at ~30 s each is six minutes of wall clock for work the AI gateway was always willing to run in parallel. Both slices now run bounded-parallel (`DISCOVERY_AI_CONCURRENCY`, default 4; the gateway's own `AI_MAX_CONCURRENCY` and RPM limiter stay the hard bounds) with **one session per in-flight verdict** (the gateway writes to the session it is handed, and a session is not safe to share between awaiting coroutines) and a **fail-fast** contract: the first failing verdict cancels the rest, so an outage cannot fan out into one failed call per candidate — or open the breaker on a single faulty run's burst (`ai_client.collapse_slice_failures` counts a cancelled slice as **one** consecutive failure, and the paused-run contract — nothing persisted, identical resumed run — is unchanged and still pinned).

The fan-out's per-source budget is now a setting rather than a buried `45.0`:
`DISCOVERY_FETCH_TIMEOUT_SECONDS` (same default, so nothing changes out of the box) lets an operator
with a slow network raise it — or lower it so one slow source is reported as `timeout` in the run
report instead of holding the run open.

The local decision engine got the same treatment from the other side: `laya.predict` now serialises inference behind one process-wide lock — the engine is a single torch model, and concurrent forward passes on one device are what produce the macOS `MTLCommandBufferStatusCommitted` assertion (two command buffers on one Metal device) and thrash memory elsewhere — and repeated engine failures **park** it (`_FAILURE_THRESHOLD` consecutive failures, 120 s cooldown, `engine_parked` as its own reported reason). Auto mode falls back to the LLM exactly as before; `laya_only` reports the outage immediately instead of waiting out another timeout; a success un-parks it. Without this, a dead engine cost a discovery run one `LAYA_TIMEOUT_SECONDS` wait *per candidate* before every one of them fell back.

### Fixed — hermetic AI tests could wedge the scripted provider on a cancelled call

The conftest scripted providers served requests from a single-threaded `HTTPServer`, so a request abandoned by a cancelled parallel slice left the serve loop blocked and the fixture's `server.shutdown()` never returned (the suite hung ~300 s in teardown, faulthandler-dumped to `conftest.py`). They now serve from a `ThreadingHTTPServer` with a per-server request lock and a socket timeout, which is what lets the parallel slices be tested at all.


### Fixed — a checkbox question is answered with a tick box, not a text input

The screenshot that started this: Assisted Apply paused on a **checkbox** question and the queue rendered *"Your answer for this field (never a password or code)"* — a free-text box standing in for a tick box. The bug was one line of UI (a single `<input>` for every non-handoff field regardless of type) but its roots ran through the whole answer path: the field verdict dropped the control's `type`/`options` metadata on the way to the action row, the browser observation read a checkbox's DOM `value` attribute (the constant `"on"` even when unticked) as "has a value" so an untouched box looked already-filled, radio groups arrived as one question per dot, and both fill drivers typed everything with `locator.fill()` — which cannot tick a box at all (auto-apply's checkbox branch only ever *checked*, so an explicit "no" was silently dropped).

**Fix:** every layer now carries and honours the control's own shape. `FieldVerdict` keeps `type` + `options` through `as_dict()` (action rows, `UserInputRequest` fields and the autofill plan all ship them); the observation script reports `value_present: checked` for checkboxes, groups radios into one field with the portal's own option labels (fieldset legend as the question where present); a new shared executor (`app/services/form_fill.py`) ticks/unticks checkboxes (`"false"` is the *answer* "no", so the box is unticked, never left alone), checks the picked radio option, selects the portal's own `<select>` option (label → value → case-insensitive match) and only uses `fill()` for text — used by both the assisted driver and the auto-apply executor (which gains radio support and explicit untick); `record_user_answers` normalises JSON booleans to `"true"`/`"false"` so a "no" survives as a real value (typed on the next pass, learned per-host like any answer) instead of collapsing into "unanswered". The UI (Assisted Apply **and** the Queues' User Input Needed form) renders one control per field from `components/AnswerField.tsx` — tick box, the portal's own option labels (radio group or picker), a date picker for dates, textarea, single-line inputs otherwise, and never a value control for a password/code (advice to use the browser instead) — and one *Save answer* now collects **every** field of a multi-field item (previously only `fields[0]` was ever sent). Covered by `backend/tests/test_typed_answer_fields.py` (**12 tests** — verdict payload shape, the executor against a fake page, boolean answers surviving to fill instructions, plan options) and 4 new Assist tests + `AnswerField.test.tsx` (**4 tests** — control-kind contract, explicit-true/false collection, secrets never collected).

### Added — Laya: the local typed-decision engine, owner-routed (ranking, classification, field mapping)

Discovery + ranking were the slow, failure-prone path: every match verdict was a *generative* LLM call whose JSON could malformed, truncate or fail guardrails ("a lot of AI failures"), serially over a discovery batch. But ranking, company-size classification and form-field mapping are **typed decisions** — a rubric score, a choice among declared options — which is exactly what **Laya** (the open, self-hostable Jev alternative; Apache 2.0, `pip install laya`) answers in one forward pass (tens of ms, ~1 GB RAM, calibrated probabilities, and structurally unable to emit malformed JSON because it never generates text).

**Strategy.** A new `app/services/laya.py` adapter owns three primitives (`choice` / `noul` / score-on-a-rubric) with normalised answers (option + calibrated confidence + distribution), runs the sync engine off the event loop under `LAYA_TIMEOUT`, sends long documents to the multilingual checkpoint with `max_len=8192` (Laya's own guidance — the English checkpoint would silently truncate a JD) and treats every engine failure as `LayaUnavailable`, never a guess. Routing is one predicate (`laya.should_attempt`): the operator ceiling `LAYA_ENABLED` ∧ the owner's mode ∧ the per-task opt-in ∧ the package being installed. Three integration points, each keeping its full LLM path as fallback: **ranking** (`scoring.laya_score_detailed` — one batched pass over the six rubric dimensions, weighted score + recommendation band computed *in code* so nothing can re-grade itself, provenance `score_source="laya"` with its own UI badge, evidence left empty rather than paraphrased), **company-size classification** (a `choice` over the four existing buckets) and **live form-field mapping** (one batched `choice` question per unmapped field; confident answers skip the LLM entirely, the rest escalate). The owner configures it in **Admin → Local decision engine (Laya)** — mode **Auto** (Laya first, the AI gateway escalates low-confidence answers and answers everything when Laya is absent — the pre-Laya behaviour byte-for-byte), **Laya only** (never fall back; an engine outage is reported as the same honest `pending`/unavailable surfaces an AI outage gets), **AI only** (never consult Laya) — plus per-task toggles, all behind the same `owner_required` gate as the AI provider category and strict boolean coercion. `run.sh` ships it (`WITH_LAYA=1` by default, `WITH_LAYA=0` to omit; `backend/requirements-laya.txt`; a failed install is a warning, never a broken boot) and **warms** it: `backend/scripts/warm_laya.py` pre-downloads the `english` + `multilingual` checkpoints into the Hugging Face cache on install (the app never routes to `typed-decisions`) and records a marker so later starts skip the step and decisions work offline — `LAYA_WARMUP=0` skips the download. Docker images opt in via `--build-arg WITH_LAYA=1` (CPU torch + the package; `LAYA_WARMUP=1`, the default there, also bakes the weights into `HF_HOME=/opt/laya-cache` for fully-offline containers — `LAYA_WARMUP=0` builds package-only and downloads at first use), passed through by both compose files; CI builds the variant and proves its cache loads with `--network none` as the runtime user. Covered by `backend/tests/test_laya_engine.py` (**15 tests** — answer normalisation, env ceiling, `llm_only` never touching the engine, `laya_only` reporting outages as `AIUnavailableError`/`pending`, band-follows-score, batched field mapping, owner/member settings gating and mode validation) against a fake router and `backend/tests/test_warm_laya.py` (**7 tests** — marker skip/merge, `--all`, offline `--check`, missing-package reporting), the full backend suite (**1379 passed**), frontend (**193 passed**), `ruff check backend`, `tsc --noEmit` and `vite build` are clean. `.env.example` and the README document the routing modes and the honest degradation.

### Added — Assisted Apply can create portal accounts for you, passwords included (opt-in)

The assisted loop's last hard refusal was the right one by default and the wrong one as a product limit: on a sign-up page (Lever, Workday, Greenhouse, …) it stopped at the password and handed the human the whole step, even though the vault already knew how to mint a credential. Two explicit, per-user **Settings toggles** (both **off by default**, each capped by a server flag) now decide:

**`Create portal accounts & fill passwords for me` (`browser.create_accounts` ∧ `BROWSER_CREATE_ACCOUNTS_ENABLED`).** When a pass's policy carries the opt-in and the observed page actually has password fields, the pass loads the portal's credential from the vault — creating one on first sight via `credential_for_application` (username = the profile email, `origin="auto"`, journal event `credential_created` with the domain only) — and injects the password into *this fill's* in-memory values. The classifier then clears the field (`allow_credentials`: credential verdicts become `autofill`, the `login` challenge pause is dropped), the fill planner builds the instruction, and the driver re-proves the same condition before typing a character. Everything else is unchanged by construction: MFA and CAPTCHA always pause; without the opt-in every layer refuses exactly as before; the password never enters `session.fill_values` or the checkpoint (fingerprint only) or the journal (field name only) or the input recorder (password fields hard-excluded) or the logs; `application.auto_create_credentials` still vetoes creation. Because the credential lands in the vault, the **existing** export (`Vault → Chrome/Apple CSV`) already carries it — no new export surface. A page whose password field the human already filled is now reported as `held` (nothing left to type, advance) instead of a false `no_field_could_be_filled`.

**`Let AI map unfamiliar form fields` (`browser.ai_assist` ∧ `BROWSER_AI_ASSIST_ENABLED`).** Flips the previously hardcoded `use_ai=False` in `prepare_form_plan`: unmapped fields go to the existing `form_detect` AI workflow, which tags each suggestion `profile_key_source: "ai"`. An AI outage propagates to the caller — it never silently degrades to heuristics; turn the toggle off for the deterministic loop.

Settings exposes both with the availability flags (`create_accounts_available` / `ai_assist_available`) so the UI disables a toggle the server would ignore, and both keys are written through the strict boolean coercion (`"false"` the string can never read as enabled; the policy gate itself is fail-closed on a literal `True`). Covered by `backend/tests/test_assisted_account_creation.py` (policy ceilings, classifier/planner/driver layers, an end-to-end opted-in pass through `RecordingDriver` with vault creation + CSV export, MFA-still-pauses, AI flag wiring — **26 tests**) plus `SettingsAssistOptIn.test.tsx` (**3 tests**); full backend suite **1327 passed**, frontend **185 passed**, `ruff`/`mypy` (118 files)/`tsc`/`vite build` all clean. `.env.example`, `docs/PRIVACY_REVIEW.md`, `docs/contracts/10-automation-policy.md` and the Assist page copy state the new behaviour honestly.

### Added — Assisted Apply opens a window you can work in, and learns what you type there (minus secrets)

Two gaps made the previous Assisted Apply fix feel incomplete in practice: pressing **Start a handoff window** opened nothing, and the product had no answer to *"how will it track my input so it can replicate later?"*

**The window.** Every pass closed its browser in a `finally` — including the instant it paused for a human step, so the promised "take over in that same window" could never happen, and a handoff only rendered a link into a *different* browser that shares no state with the automation. Now a session-scoped registry (`app.services.live_browser`) keeps a **headed** window alive while a human step is pending: `POST …/actions/{id}/handoff` opens the session's own Playwright window on the step's URL (`window_opened` in the descriptor says honestly whether one appeared — headless hosts still get the link plus the reason), a paused pass keeps its window registered instead of closing it, the next pass (queue, `/pass`, or after **I finished this step**) *adopts* the same window rather than launching a new browser, and completing a handoff exports the window's cookie jar (when `BROWSER_SESSION_PERSIST` is on) so the continuing pass resumes **signed in** instead of meeting the login wall again. Cancel, re-authentication, portal confirmation, process shutdown and a 20-minute idle sweep all close windows explicitly; headless runs behave exactly as before (close at once) and a window the user closed themselves is detected and replaced. The session payload carries `window: {open, idle_seconds, age_seconds}` and the UI marks the session whose window is open.

**The input.** The shared window installs a conservative recorder: `change` events only (a committed field, never a keystroke stream), never password/hidden/file fields or `one-time-code`/`current-password` autocomplete — re-checked server-side by name, by type and finally by the classifier's own restricted verdict (a field like "Government ID" is refused even when the name looks innocent). An accepted field lands four ways: the portal's identity for it (structure), checkpoint status `user_completed` + a fingerprint (never the value), the session working set, and a bounded per-host **learned answers** store (user setting `browser.learned_answers`, capped at 300 entries) that future sessions for the same host merge into their fill values — session answers outrank learned ones, learned outrank the static plan, and another employer's portal never borrows them. The in-app *Save answer* path learns the same way. The flow journal gains one `fill` entry per field with `reason=user_in_browser` — the name, never the value.

**Verification.** 23 new backend tests (`backend/tests/test_live_browser_and_input_recording.py`) + 4 new frontend tests; full backend suite **1301 passed**, frontend **182 passed**, `ruff check backend`, `mypy app`, `tsc --noEmit` and `npm run build` clean. `backend/scripts/e2e_assisted_window.py` drives a real Chromium against a local portal on a desktop machine — it proves the headed launch (with a screenshot), the keep-open pause, the recording/refusal split, the cookie export and the same-window second pass; on a server it stops at the honest `mode_reason` instead of pretending.
